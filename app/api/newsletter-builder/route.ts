...
export async function POST(request: NextRequest) {
  ...
  // ── 5. Feature lock check ────────────────────────────────────
  if (isLocked(subscription.plan as Plan, 'newsletters'))
    return NextResponse.json({ error: 'Newsletters are locked on your plan. Upgrade to Pro.' }, { status: 403 })
  ...
  try {
    ...
    if (error.message === 'CAPACITY_EXCEEDED')
      return NextResponse.json({ error: 'Generation capacity exceeded.' }, { status: 503 })
